...
# The entire editor HTML/CSS/JS as a raw string
html_content = r"""
<!DOCTYPE html>
<html lang="en">
<head>
  ...
  <style>
    * { margin:0; padding:0; box-sizing:border-box; }
    body { font-family:'Segoe UI',Tahoma,sans-serif; background:#fff; color:#333; }
    .container { padding:20px; }
    .header { text-align:center; margin-bottom:20px; border-bottom:2px solid #000; padding-bottom:10px; }
    .toolbar {
      background:#f5f5f5; padding:10px; border:1px solid #ddd; border-radius:5px;
    ...
    .toolbar .table-props input {
      width:60px; padding:2px 4px; font-size:12px;
    }
    .toolbar .table-props button {
      font-size:12px; padding:4px 8px;
    }
    .divider { height:2px; background:#000; margin:15px 0; }
    .editor-container {
      display:grid; grid-template-columns:1fr 1fr; gap:20px; height:600px;
    }
    .editor-panel { border:1px solid #ddd; border-radius:5px; display:flex; flex-direction:column; }
    .panel-header {
      background:#f0f0f0; padding:10px; font-weight:bold; border-bottom:1px solid #ddd;
    }
    .editor, .html-editor {
      flex:1; padding:15px; outline:none; font-size:14px; line-height:1.6; overflow-y:auto;
    }
    .editor { border:none; }
    .html-editor {
      border:1px solid #ddd; background:#f8f8f8; font-family:Consolas,'Courier New',monospace;
      white-space:pre-wrap;
    }
    .table-controls {
      display:none; position:fixed; background:#fff; border:2px solid #007acc;
    ...
    .table-controls input {
      width:100%; margin-bottom:10px; padding:5px; border:1px solid #ccc; border-radius:3px;
      font-size:12px;
    }
    .table-controls button {
      margin-right:5px; padding:8px 12px; border:none; border-radius:3px; color:#fff;
      background:#007acc; cursor:pointer; font-size:12px;
    }
    .table-controls button:hover { background:#005999; }
    .table-controls .close-btn { background:#ccc; color:#333; }
    table.selected {
      outline:2px solid #007acc; background:rgba(0,122,204,0.1);
    }
  </style>
</head>
<body>
  ...
  <script>
    ...
    const tablePropsDiv = document.querySelector('.table-props');
    ...
    function insertLink() {
      const url = prompt('URL:','https://'), txt = prompt('Link text:','Link');
      if(url) execCmd('insertHTML', `<a href="${url}" target="_blank">${txt}</a>`);
    }
    function insertImage() {
      const url = prompt('Image URL:''), alt = prompt('Alt text:','');
    ...
    function pasteAsPlainText() {
      if(navigator.clipboard && navigator.clipboard.readText){
        navigator.clipboard.readText()
          ...
          .catch(_=>{ const t=prompt('Paste text:'); if(t) execCmd('insertText',t); });
      } else {
        const t=prompt('Paste text:'); if(t) execCmd('insertText',t);
      }
    }
    ...
    function loadFile(e) {
      ...
      r.onload=ev=>{ editor.innerHTML=ev.target.result; updateHTML(); };
      r.readAsText(f);
    }
    ...
    function handleTableSelection(e){
      ...
      if(tbl){
        selectedTable=tbl; tbl.classList.add('selected');
    ...
    function showTableControls(x,y){
      ...
      // fill toolbar
      propWidth.value       = selectedTable.getAttribute('width')||'';
      propBorder.value      = selectedTable.getAttribute('border')||'';
      propCellSpacing.value = selectedTable.getAttribute('cellspacing')||'';
      propCellPadding.value = selectedTable.getAttribute('cellpadding')||'';
    }
    ...
    function applyTableSettings(){
      ...
      selectedTable.querySelectorAll('td,th').forEach(c=>c.style.padding=`${cp}px`);
    ...
    document.addEventListener('keydown',e=>{
      if((e.ctrlKey||e.metaKey)&&!e.shiftKey){
        const m={b:'bold',i:'italic',u:'underline'};
        if(m[e.key]){ e.preventDefault(); execCmd(m[e.key]); }
      }
    });

    document.addEventListener('click',e=>{
      const ctl=document.getElementById('tableControls');
      if(ctl.style.display==='block' && !ctl.contains(e.target) && !e.target.closest('table')){
        closeTableControls();
      }
    });
